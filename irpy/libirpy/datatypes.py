...
class Reference(object):
    ...
    def build_field_tuple_and_path(self, ctx, path):
        typ = self._type
        #print "\nirpy/libirpy/datatypes.py.Reference.py.build_field_tuple_and_path***************"
        #print "self._type: {}".format(self._type)
        #if typ.is_array():
            #print "\nself._type.length(): {}".format(self._type.length())
        #print "\npath: {}".format(path)
        #print "\npath[0]: {}".format(path[0])
        #print "*********************"
        fields = []
        newpath = []
        while len(path) > 0:

...
extern struct k_msgq my_msgq_callback;
 ...
 */
tU4 k5_call(tK5_esb *esb, /*ESB总线数据结构*/
	    tU2 service, /*服务编码*/
...
)
{
	...
	if (svc_num <= 0 || svc_num > K5_LAST_SVC)
		return K5_NO_SERVER;
	if (esb == NULL || to == NULL)
		return K5_NO_ACCESS;
 ...
 *    K5_COMPLETE : 原语调用成功,但不为输入型服务或指定缓冲区不够大时
 */
tU4 k5_wait(tK5_esb *esb, /*ESB总线数据结构*/
...
)
{
		...
		if (s_len > 0 && s_buf != NULL) /*若有服务结果数据送回*/
	{
		...
		esb->size =
			(esb->head + 1) * 8 + s_len; /*设置帧长度 = 头长+体长*/
	};
 ...
 */
// tU4 k5_send(tK5_esb *esb, /*ESB总线数据结构*/
// 	    tU2 service, /*服务编码*/
// 	    tK5_net *to, 目的端口号（pid,fd,sock）及网络地址
...
// )
{
	...
	if (s_len > 0 && s_buf != NULL) /*若有数据要发送,拷贝到ESB*/
	{
		memcpy(&esb->body[esb->head], s_buf, s_len);

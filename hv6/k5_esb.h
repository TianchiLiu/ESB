 ...
 * @param  tK5_esb *esb : 包含服务id和待处理数据的事件帧结构，该参数不为NULL
 * @param  tU2      service : 服务编码,该参数不能越界
 * @param  tK5_net      *to : 目的端口号（pid,fd,sock）及网络地址,该参数不为NULL
 * @param  tU4        c_len : 同步调用原语的缓冲区长度(长字)
 * @param  tU4       *c_buf : 同步调用原语的缓冲区地址 
 * @return K5_NO_SERVER : 传入的系统服务id信息越界
 * @return K5_NO_ACCESS : 传入的事件帧或目的信息或网络地址不存在
 * @return esb->size : 原语调用成功,且为输入型服务且指定缓冲区足够大时,返回收到的帧结构大小
 * @return K5_COMPLETE : 原语调用成功,但不为输入型服务或指定缓冲区不够大时
 */
tU4 k5_call( //5个参数
 ...
 * @param tK5_esb *esb : 包含服务id和待处理数据的事件帧结构，该参数不为NULL
 * @param tK5_net      *from : 指定期望的端口号及网络地址，该参数不为NULL
 * @param tU4          w_len : 接收缓冲区长度，使用零拷贝时不用
 * @param tU4         *w_buf : 接收缓冲区地址，使用零拷贝时不用
 * @return　K5_NO_ACCESS : 传入的事件帧或指定期望的端口号或网络地址不存在
 * @return esb->size : 原语调用成功,且为输入型服务且指定缓冲区足够大时,返回收到的帧结构大小
 * @return K5_COMPLETE : 原语调用成功,但不为输入型服务或指定缓冲区不够大时
 */
 ...
 * @param tK5_esb *esb : 包含服务id和待处理数据的事件帧结构，该参数不为NULL
 * @param   tU2   ack_err : 为正数表示确认序列号，为负数表示错误编号；
 * @param   tU4    s_len : 发送缓冲区长度,按8字节计，含帧头；
 * @param   tU4   *s_buf : 发送缓冲区地址,带服务结果数据；
 * @return　K5_NO_ACCESS : 传入的事件帧为NULL
 * @return  esb->size : 原语调用成功,返回帧结构大小
 *  
 */
 ...
 * @param tU2      service : 服务编码,该参数不能越界
 * @param tK5_net      *to : 目的端口号（pid,fd,sock）及网络地址,该参数不为NULL
 * @param tU4        s_len : 同步调用原语的缓冲区长度(长字)
 * @param tU4       *s_buf : 同步调用原语的缓冲区地址 
 * @return K5_NO_SERVER : 传入的系统服务id信息越界
 * @return　K5_NO_ACCESS : 传入的事件帧或目的信息或网络地址不存在
 * @return esb->size : 原语调用成功,返回发送的帧结构大小
 */
 ...
 * @param tU4        c_len : 同步调用原语的缓冲区长度(长字)
 * @param tU4       *c_buf : 同步调用原语的缓冲区地址 
 * @return KK_NO_SERVER : 传入的系统服务id信息越界
 * @return　KK_NO_ACCESS : 传入的事件帧或目的信息或网络地址不存在
 * @return esb->size : 原语调用成功,且为输入型服务且指定缓冲区足够大时,返回收到的帧结构大小
 * @return KK_COMPLETE : 原语调用成功,但不为输入型服务或指定缓冲区不够大时
 */
tU4 kk_call( //5个参数
	tK5_esb *esb, //ESB总线数据结构（改为第1个参数）
	tU2 service, //服务编码
 ...
 * @param tK5_esb *esb : 包含服务id和待处理数据的事件帧结构，该参数不为NULL
 * @param tK5_net      *from : 指定期望的端口号及网络地址
 * @param tU4          w_len : 接收缓冲区长度，使用零拷贝时不用
 * @param tU4         *w_buf : 接收缓冲区地址，使用零拷贝时不用
 * @return　KK_NO_ACCESS : 传入的事件帧或指定期望的端口号或网络地址不存在
 * @return  esb->size : 原语接收成功,且指定缓冲区足够大时,返回收到的帧结构大小
 * @return KK_COMPLETE : 原语接收成功,但指定缓冲区不够大时
 */
 ...
 * @param   tU4    s_len : 发送缓冲区长度,按8字节计，含帧头；
 * @param   tU4   *s_buf : 发送缓冲区地址,带服务结果数据；
 * @return　KK_NO_ACCESS : 传入的事件帧为NULL
 * @return  esb->size : 原语调用成功,返回帧结构大小
 *   
 */
...
#define console_print 0x00bd //控制台输出信息

#define K5_LAST_SVC 0x00bd //实际最大服务编码
 ...
 */
/**
 * @brief search_server_num()函数描述
 * 功能： 根据帧结构中的服务id信息查找系统服务向量表并调用系统服务接口，根据帧结构传入数据完成相应系统服务
 * @param  tK5_esb *esb : 包含服务id和待处理数据的事件帧结构，该参数不为NULL
 * @return  KK_COMPLETE : 完成相应的系统服务调用
 * @return  KK_NO_SERVER : 传入的系统服务id错误
 */
int search_server_num(tK5_esb *esb);

/**
 * @brief esb_server()函数描述
 * 功能：esb总线的主函数，接收事件并根据事件的类型做出相应的事件驱动操作
 * @param  无
 * @return KK_ERR_WAIT : 接收到的帧结构地址为NULL
 * @return KK_NO_DST : 接收到的帧结构内无所需的源地址信息
 * @return KK_NO_SRC : 接收到的帧结构内无所需的目标地址信息
 * @return KK_NO_PRIMITIVE : 接收到的帧结构类型错误
 */
int esb_server();

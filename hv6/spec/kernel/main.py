...
def newctx():
    ...
    ctx.ptr_to_int[ctx.globals['@esb_table']._ref._name] = util.FreshBitVec( '(uintptr)@esb_table', 64)   #/*new*/
...
#add:疑问?:没有用到 Test cases to check if python and c versions of implementation invariants match
...
class HV6TopLemmas(HV6Base):
    ...
    def setUp(self):
        conj = []
        print "main.py.HV6TopLemmas:entering...setUp"
        self.solver = Solver()
        self.solver.set(AUTO_CONFIG=False)
